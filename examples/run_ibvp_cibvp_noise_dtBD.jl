...
dtψv1_BD(t::T, z::T, ibvp::noise_ibvp) where {T<:Real} =
    ibvp.noise_amp_ψv1 * randn(T)
#model_CCE_CCM.
dtψ1_BD(t::T, z::T, ibvp::noise_ibvp) where {T<:Real} =
    ibvp.noise_amp_ψ1 * randn(T)
...
#model_CCE_CCM.
ϕ2_BD(t::T, z::T, cibvp::noise_cibvp) where {T<:Real} =
    cibvp.noise_amp_ϕ2 * randn(T)
#model_CCE_CCM.
ψv2_BD(t::T, z::T, cibvp::noise_cibvp) where {T<:Real} =
...
toy_model = "SYMH_SYMH_noise_t20_q_amp"
root_dir="./run_ibvp_cibvp_dtBD/"
...
cibvp = noise_cibvp(
    # for given data that converge in q-norm, a->b ϕ1 and ϕ2 amp; for H1, a->b everywhere
    noise_amp_ϕ2  = noise_amplitude_drop_b^D,
...
run_ibvp_cibvp_dtBD(p, ibvp, cibvp)
