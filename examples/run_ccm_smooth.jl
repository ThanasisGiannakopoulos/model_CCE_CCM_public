...
# Cauchy initial data
model_CCE_CCM.ϕ1_ID(ρ::T, z::T, ibvp::smooth_ibvp) where {T<:Real} =
    1.0*ID_gauss_IBVP(ρ, z)
model_CCE_CCM.ψv1_ID(ρ::T, z::T, ibvp::smooth_ibvp) where {T<:Real} =
    1.0*ID_gauss_IBVP(ρ, z)
model_CCE_CCM.ψ1_ID(ρ::T, z::T, ibvp::smooth_ibvp) where {T<:Real} =
    1.0*ID_gauss_IBVP(ρ, z)
# Cauchy boundary data
# right-moving:
model_CCE_CCM.ϕ1_BD(t::T, z::T, ibvp::smooth_ibvp) where {T<:Real} =
    in_gauss_various_pulses(t,z)
#0.0 # it is not compatible with ID, but we neglect the diff, it is less that 10^-16
model_CCE_CCM.ψv1_BD(t::T, z::T, ibvp::smooth_ibvp) where {T<:Real} =
    in_gauss_various_pulses(t,z)
    #0.0
# the left-moving BD for the IBVP is given by the solution to the CIBVP for CCM
# characteristic initial data
model_CCE_CCM.ψ2_ID(x::T, z::T, cibvp::smooth_cibvp) where {T<:Real} =
    1.0*ID_gauss_CIBVP(x, z)
# characteristic boundary data
# left-moving:
model_CCE_CCM.ψ2_BD(t::T, z::T, cibvp::smooth_cibvp) where {T<:Real} =
    in_gauss_various_pulses(t,z)
#0.0
...
mkpath(par_copy)
cp("./run_ccm_smooth.jl", par_copy, force=true)
