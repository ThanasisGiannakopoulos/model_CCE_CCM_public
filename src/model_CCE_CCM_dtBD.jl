#module model_CCE_CCM_dtBD

#export Param, IBVP, CIBVP # needed to read parameters of the discretization and of the PDE structure for the IBVP and CIBVP
#export run_ibvp_cibvp_dtBD, run_cce_dtBD, run_ccm_dtBD # exports functions that are called during the run of an example
...
"""
function dtϕ1_BD end
function dtψv1_BD end
function dtψ1_BD end
...
function setup_rhs_ibvp_cibvp_dtBD!(t::Float64,
    ...
    @inbounds for j in 1:Nz
        ...
        dv[1,j,1] = dtϕ1_BD(t, sys.z[j], ibvp)
        dv[1,j,2] = dtψv1_BD(t, sys.z[j], ibvp)
        # incoming boundary data from the right boundary of the characteristic
        dv[end,j,4] = dtψ2_BD(t, sys.z[j], cibvp)

        """ This part is different for CCM, CCE, and IBVP_CIBVP"""
        # BD from right boundary of Cauchy, that touches characteristic domain
        dv[end,j,3] = dtψ1_BD(t, sys.z[j], ibvp) # prescribed BD from example file; it does NOT come from the CIBVP
    ...
    #dϕ1/dt; right moving
    dv[2:end,:,1] .= -ibvp.vϕ1*Dρ(v[:,:,1], sys)[2:end,:] .+
        ibvp.az11*Dz(v[2:end,:,1], sys) .+ibvp.az12*Dz(v[2:end,:,2], sys) .+
        ibvp.az13*Dz(v[2:end,:,3], sys) .+
        ibvp.b11*v[2:end,:,1] .+ ibvp.b12*v[2:end,:,2] .+ ibvp.b13*v[2:end,:,3] 
    #dψv1/dt; right moving
    dv[2:end,:,2] .= -ibvp.vψv1*Dρ(v[:,:,2], sys)[2:end,:] .+
        ibvp.az21*Dz(v[2:end,:,1], sys) .+ibvp.az22*Dz(v[2:end,:,2], sys) .+
        ibvp.az23*Dz(v[2:end,:,3], sys) .+
        ibvp.b21*v[2:end,:,1] .+ ibvp.b22*v[2:end,:,2] .+ ibvp.b23*v[2:end,:,3] 
    #dψ1/dt; left moving
    dv[1:end-1,:,3] .= ibvp.vψ1*Dρ(v[:,:,3], sys)[1:end-1,:] .+
        ibvp.az31*Dz(v[1:end-1,:,1], sys) .+ibvp.az32*Dz(v[1:end-1,:,2], sys) .+
        ibvp.az33*Dz(v[1:end-1,:,3], sys) .+
        ibvp.b31*v[1:end-1,:,1] .+ ibvp.b32*v[1:end-1,:,2] .+ ibvp.b33*v[1:end-1,:,3]
    #dψ2/du in characteristic; left moving
    dv[1:end-1,:,4] .= cibvp.vψ2* DX(v[:,:,4], sys)[1:end-1,:] .+
        cibvp.az13*Dz(null_v[1:end-1,:,1], sys) .+ cibvp.az23*Dz(null_v[1:end-1,:,2], sys) .+
        cibvp.az33*Dz(v[1:end-1,:,4], sys) .+
        cibvp.b31*null_v[1:end-1,:,1] .+ cibvp.b32*null_v[1:end-1,:,2] .+ cibvp.b33*v[1:end-1,:,4]
...
function time_evol_ibvp_cibvp_dtBD!(t::Float64, dt::Float64,
                    v::Array{Float64, 3},
    ...
    # time evol with RK4:
    #v1 .=
    setup_rhs_ibvp_cibvp_dtBD!(t, v1, v,
               null_v,
               null_v1, null_v2,
               sys, ibvp, cibvp)
    #v2 .=
    setup_rhs_ibvp_cibvp_dtBD!(t, v2, v .+ 0.5.*dt.*v1,
               null_v,
               null_v1, null_v2,
               sys, ibvp, cibvp)
    #v3 .=
    setup_rhs_ibvp_cibvp_dtBD!(t, v3, v .+ 0.5.*dt.*v2,
               null_v,
               null_v1, null_v2,
               sys, ibvp, cibvp)
    #v4 .=
    setup_rhs_ibvp_cibvp_dtBD!(t, v4, v .+ dt.*v3,
               null_v,
               null_v1, null_v2,
               sys, ibvp, cibvp)

    # time evolve; the standard RK4 scheme
    # v1,v2,v3,v4 are prescribed data (time derivatives)
    v .= v .+ dt*(v1 .+ 2.0.*v2 .+ 2.0.*v3 .+ v4)/6.0

    #  boundary data for the null integration
    @inbounds for j in 1:Nz
...
function run_ibvp_cibvp_dtBD(p::Param, ibvp::IBVP, cibvp::CIBVP)
    ...
    println("Running IBVP and CIBVP (dtBD) separately")
    println("-------------------------------------------------------------")
    ...
    println("-------------------------------------------------------------")
    println("Done.")
end # end run_ibvp_cibvp_dtBD

##################################################################################################
# CCE
##################################################################################################

function setup_rhs_cce_dtBD!(t::Float64,
                    dv::Array{Float64, 3}, # dv/dt for  [ϕ1, ψv1, ψ1, ψ2]; d/du for characteristic
                    v::Array{Float64, 3}, # vector v = [ϕ1, ψv1, ψ1, ψ2]
                    null_v::Array{Float64, 3}, # vector null_v = [ϕ2, ψv2]
                    null_v1::Matrix, null_v2::Matrix, # used for null integration
    ...
    @inbounds for j in 1:Nz
        ...
        # BD from right boundary of Cauchy, that touches characteristic domain
        dv[end,j,3] = dtψ1_BD(t, sys.z[j], ibvp) # for CCE: prescribed BD for the left-moving field of the IBVP
    ...
    #dϕ1/dt; right moving
    dv[2:end,:,1] .= -ibvp.vϕ1*Dρ(v[:,:,1], sys)[2:end,:] .+
        ibvp.az11*Dz(v[2:end,:,1], sys) .+ibvp.az12*Dz(v[2:end,:,2], sys) .+ibvp.az13*Dz(v[2:end,:,3], sys) .+
        ibvp.b11*v[2:end,:,1] .+ ibvp.b12*v[2:end,:,2] .+ ibvp.b13*v[2:end,:,3] 
    #dψv1/dt; right moving
    dv[2:end,:,2] .= -ibvp.vψv1*Dρ(v[:,:,2], sys)[2:end,:] .+
        ibvp.az21*Dz(v[2:end,:,1], sys) .+ibvp.az22*Dz(v[2:end,:,2], sys) .+ibvp.az23*Dz(v[2:end,:,3], sys) .+
        ibvp.b21*v[2:end,:,1] .+ ibvp.b22*v[2:end,:,2] .+ ibvp.b23*v[2:end,:,3] 
    #dψ1/dt; left moving
    dv[1:end-1,:,3] .= ibvp.vψ1*Dρ(v[:,:,3], sys)[1:end-1,:] .+
        ibvp.az31*Dz(v[1:end-1,:,1], sys) .+ibvp.az32*Dz(v[1:end-1,:,2], sys) .+ibvp.az33*Dz(v[1:end-1,:,3], sys) .+
        ibvp.b31*v[1:end-1,:,1] .+ ibvp.b32*v[1:end-1,:,2] .+ ibvp.b33*v[1:end-1,:,3]
    #dψ2/du in characteristic; left moving
    dv[1:end-1,:,4] .= cibvp.vψ2* DX(v[:,:,4], sys)[1:end-1,:] .+
        cibvp.az13*Dz(null_v[1:end-1,:,1], sys) .+ cibvp.az23*Dz(null_v[1:end-1,:,2], sys) .+
...
function time_evol_cce_dtBD!(t::Float64, dt::Float64,
                    v::Array{Float64, 3},
                    v1::Array{Float64, 3}, v2::Array{Float64, 3},
                    v3::Array{Float64, 3}, v4::Array{Float64, 3},
                    null_v::Array{Float64, 3},
    ...
    # time evol with RK4:
    #v1 .=
    setup_rhs_cce_dtBD!(t, v1, v,
               null_v,
               null_v1, null_v2,
               sys, ibvp, cibvp)
    #v2 .=
    setup_rhs_cce_dtBD!(t, v2, v .+ 0.5.*dt.*v1,
               null_v,
               null_v1, null_v2,
               sys, ibvp, cibvp)
    #v3 .=
    setup_rhs_cce_dtBD!(t, v3, v .+ 0.5.*dt.*v2,
               null_v,
               null_v1, null_v2,
               sys, ibvp, cibvp)
    #v4 .=
    setup_rhs_cce_dtBD!(t, v4, v .+ dt.*v3,
               null_v,
               null_v1, null_v2,
               sys, ibvp, cibvp)
...
function run_cce_dtBD(p::Param, ibvp::IBVP, cibvp::CIBVP)
    ...
    println("-------------------------------------------------------------")
    println("Done.")
end # end run_cce_dtBD

##################################################################################################
# CCM
##################################################################################################

function setup_rhs_ccm_dtBD!(t::Float64,
                    dv::Array{Float64, 3}, # dv/dt for  [ϕ1, ψv1, ψ1, ψ2]; d/du for characteristic
                    v::Array{Float64, 3}, # vector v = [ϕ1, ψv1, ψ1, ψ2]
                    null_v::Array{Float64, 3}, # vector null_v = [ϕ2, ψv2]
                    null_v1::Matrix, null_v2::Matrix, # used for null integration
    ...
    # BD from right boundary of Cauchy, that touches characteristic domain
    dv[end,:,3] .= dv[1,:,4] # for CCM: ψ1[end,j] = ψ2[1,j]; left-moving, pass information from the characteristic to the Cauchy domain

    nothing
end

function time_evol_ccm_dtBD!(t::Float64, dt::Float64,
                    v::Array{Float64, 3},
                    v1::Array{Float64, 3}, v2::Array{Float64, 3},
                    v3::Array{Float64, 3}, v4::Array{Float64, 3},
                    null_v::Array{Float64, 3},
    ...
    # time evol with RK4:
    #v1 .=
    setup_rhs_ccm_dtBD!(t, v1, v,
               null_v,
               null_v1, null_v2,
               sys, ibvp, cibvp)
    #v2 .=
    setup_rhs_ccm_dtBD!(t, v2, v .+ 0.5.*dt.*v1,
               null_v,
               null_v1, null_v2,
               sys, ibvp, cibvp)
    #v3 .=
    setup_rhs_ccm_dtBD!(t, v3, v .+ 0.5.*dt.*v2,
               null_v,
               null_v1, null_v2,
               sys, ibvp, cibvp)
    #v4 .=
    setup_rhs_ccm_dtBD!(t, v4, v .+ dt.*v3,
               null_v,
               null_v1, null_v2,
               sys, ibvp, cibvp)
...
# function that performs the time evolution
function run_ccm_dtBD(p::Param, ibvp::IBVP, cibvp::CIBVP)
